...
>
> Under Modality `jira-native` (no Xray) there are no separate ATP/ATR issues, so `test_design` / `test_execute` are N-A; the Story is linked directly to each Test via the generic `test` edge instead. `test_automation` stays an optional refinement in both modalities.

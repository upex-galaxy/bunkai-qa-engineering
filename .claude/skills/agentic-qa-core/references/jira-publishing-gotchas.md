...
This file is cited from `sprint-testing/references/reporting-templates.md` (ATR + bug evidence rich-text writes), `sprint-testing/references/acceptance-test-planning.md` (ATP body publish to Story), `test-documentation/references/jira-setup.md` (custom-field configuration + initial body writes), `test-documentation/references/jira-test-management.md` (Modality B Test issue body writes), `test-documentation/references/tms-architecture.md` (Test / TestPlan / TestExecution body writes), and `shift-left-testing/references/atp-draft-template.md` (ATP DRAFT body publish during pre-sprint refinement). Any workflow that publishes rich-text content to Jira must link here for the two gotchas above before issuing its first write.

...
| AI agent    | Codex (primary), OpenCode (alt)        |
...
```bash
gentle-ai install --components engram,sdd --agent <Codex|opencode|cursor>
```
...
Plus 3 project-level community skills installed into `.Codex/skills/` (not committed): `playwright-cli`, `playwright-best-practices`, `resend-cli`. See `cli/install.ts` `PROJECT_LEVEL_SKILLS` and `USER_LEVEL_SKILLS` arrays.

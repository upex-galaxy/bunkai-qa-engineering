...
| State machine | YES | `auth.users` + `magic_link_tokens` lifecycle |
...
**Flag C carry-forward:** dev comment 2026-05-27 says replay enforcement is Supabase-native (no custom `TOKEN_USED` envelope). `TC-OUT-NEG-03` keeps its expected `?error=TOKEN_USED` per shift-left §3 + adds an "Observed (Stage 2)" placeholder so Stage 2 can record divergence as gap-vs-bug.
...
| Credentials | Read from `.env` only — `STAGING_USER_EMAIL`, `STAGING_USER_PASSWORD`, `RESEND_API_KEY`. Never hardcode. |
...
| Tool | `[AUTOMATION_TOOL]` Playwright CLI + `resend` CLI + `[DB_TOOL]` DBHub MCP |
...
| Tool | `[AUTOMATION_TOOL]` Playwright CLI + `[DB_TOOL]` DBHub MCP |
...
| Steps | 1. On `/onboarding` form, accept pre-filled defaults. 2. Click "Create workspace". 3. Wait for redirect to `/projects`. 4. Query DB via DBHub: `SELECT * FROM workspaces WHERE owner_user_id = '<new-user-id>'`. 5. Query DB: `SELECT * FROM workspace_members WHERE user_id = '<new-user-id>'`. |
| Expected | RPC `bunkai_bootstrap_workspace` returns `workspace_id`. `workspaces` row exists with `owner_user_id` = the new user. `workspace_members` row exists with `role='owner'` AND `status='active'`. User redirected to `/projects`. Both inserts performed atomically (no partial state). |
| Notes | RPC is SECURITY DEFINER in `supabase/migrations/0006_bootstrap_workspace.sql`. Atomicity = no orphaned workspace row if member insert fails. |
...
| Expected | HTTP 400. Envelope shape: `{ok:false, error:{code:"INVALID_EMAIL", message:"..."}}`. |
| Notes | If server returns generic `bad_request` instead of `INVALID_EMAIL` → flag as gap per shift-left §2.3 decision. Verify against `app/api/v1/auth/magic-link/route.ts` Zod schema. |
...
| Notes | **Flag C.** If observed != expected, Stage 2 decides bug vs scope-cut. Likely Supabase returns a generic `flow_state_not_found` or `otp_already_used` — see PO Q10 + Dev Q6. |
...
| Tool | `[API_TOOL]` curl + browser via `[AUTOMATION_TOOL]` for UI assert |
...
| Tool | `[API_TOOL]` curl + `[AUTOMATION_TOOL]` Playwright CLI for toast assert |
...
| Expected | 6th request: HTTP 429. Envelope: `{ok:false, error:{code:"RATE_LIMITED", message:"..."}}`. UI shows "Too many requests — try again in N seconds" toast. |
| Notes | If Supabase returns 429 but the route maps it to a generic `upstream_error` instead of `RATE_LIMITED` → flag as gap per shift-left §2.3. |
...
| Tool | `[API_TOOL]` curl primary; `[AUTOMATION_TOOL]` Playwright secondary for UI |
| Preconditions | None. |
...
| Tool | `[AUTOMATION_TOOL]` Playwright CLI + `[API_TOOL]` curl |
| Preconditions | None. |
...
| Expected | Client rejects (no POST). Server rejects with envelope `{ok:false, error:{code:"INVALID_EMAIL", message:"Email exceeds RFC 5321 254-char limit."}}`. |
...
| Tool | `[API_TOOL]` curl + `[AUTOMATION_TOOL]` Playwright CLI to confirm landing |
...
| Steps | 1. Before bootstrap: `SELECT COUNT(*) FROM workspaces WHERE owner_user_id = <new-user>` → 0; same for `workspace_members`. 2. Trigger bootstrap form submit. 3. After submit: re-query both tables. 4. Verify timestamps `created_at` are within ~50ms of each other (atomicity heuristic). |
| Expected | Both rows present after bootstrap. Both rows absent before. If only one row appears → atomicity bug. |
| Notes | Complements TC-BK-2-03 (which is UI+DB). This is DB-only verification of the RPC contract. Migration `0006_bootstrap_workspace.sql`. |
...
| Result | Verdict |
|--------|---------|
| All 10 P1 PASS | GO (Stage 3 transitions `qa_sign_off`: in_test → qa_approved) |
...
| Any P1 FAIL | NO-GO (Stage 3 transitions `defect_reported`: in_test → blocked; bug filed) |
...
6. Supabase error code mapping — observe actual codes returned by `exchangeCodeForSession` on (a) replay (b) expiry. Record verbatim.
7. `auth.otp_exp = 900` ops checklist — verify on staging by sending a link + waiting >15 min (TC-BK-2-07).
8. Bootstrap RPC race — double-click `/onboarding` "Create workspace" → expected `23505` surfacing as "slug taken" UI message.
...
10. Replay enforcement strategy — Supabase-native handling; what is the actual error envelope returned to UI? Does it map to `?error=TOKEN_USED` (shift-left §2.3) or is there a UX gap? **Answered partly by TC-BK-2-06 observed value.**
11. Where is `MVP-NOTES.md`? Dev referenced `.context/PBI/epics/EPIC-BK-1-tenancy-identity/MVP-NOTES.md` on the target repo. Best-effort fetch via `../upex-bunkai-tms/...` during Stage 2 if testability depth requires it.
12. `/qa` route + Epic BK-29 testability guide — explore during Stage 2 smoke for QA testability shortcuts (data-testid map, seed users, clock-mock).
...
*Single source of truth: this file. Jira mirror: customfield `acceptance_test_plan` on BK-2 + Stage 1 announce comment. Stage 2 reads this; Stage 3 reads the ATR.*
